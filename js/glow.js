/* GlowCursor, ported from React Bits to a vanilla ES module and retuned
   for this site's palette: a butter-to-red luminous trail following the
   pointer, screen-blended over the page. Dependency: ogl (CDN). */
...
function init() {
  ...
  container.className = 'glow-trail';
  ...
  const render = (now) => {
    ...
    const fadeTarget = initialized && idleFor <= CONFIG.idleTimeout ? 1 : 0;
